    ...
    if _args.mode == "demultiplex":
        ...
        if _args.in_file_list is None:
            if _args.in_file is not None:
                ...
                post_process([output_dir],_args.aligner,_args.clean_up,_args.min_prop_reads)
        ...
        if _args.in_file is not None and _args.oligos is not None:
            ...
            post_process(output_dirs,_args.aligner,_args.clean_up,_args.min_prop_reads)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-F', '--primer_forward', help='Sequence of the primer forward. It will be used on consensus mode.')
    parser.add_argument('-R', '--primer_reverse', help='Sequence of the primer reverse. It will be used on consensus mode.')
    ...
    parser.add_argument('-p', '--sequence_max_mask', type=int, help='Number of mask_char allowed in sequences to be aligned. Default: 10.', default="10")
    parser.add_argument('-d', '--max_len_delta', type=int, help='Allowed maximum variation from mode of sequence length. Default: 5.', default="5")
    ...
    parser.add_argument('--min_SNP_cov', type=int, help='Minimum coverage (number of counts) to consider a single nucleotide polymorphism (SNP). In case indel option is T, indels will be trated as SNPs. All polimorphic bases with less than Otherwise the variant will be masked. Default: 5', default="5")
    parser.add_argument('--indel', help='Consider an indel as a polymorphic site. This option must be used along with min_SNP_cov. Possible options T or F. WARNING. Indels might be masked. Default: F', default='F')
