...
def process_fastq(input_fn, primer_a, primer_b, output_dir, primer_mismatch, min_base_score, min_seq_score, min_seq_cov,
                  ...
                  mask_char, consensus_ambiguous_char, consensus_ignore_mask_char,min_SNP_cov,indel,max_depth_sample):
    # parse filename
    ...
    # write trimmed and clean fasta files. Use max_depth_sample to select random sequences and avoid overloading the system and excessive runtime
    fasta_trim_cleaned_path = os.path.join(output_dir, basename) + "_Q" + str(min_base_score) + "_MaxDep" + str(max_depth_sample) + ".fasta"

    number_cleaned_seq = len(clean_records)
    if number_cleaned_seq == 1:
        r = clean_records[0]
        mask_count = r.seq.upper().count(mask_char)
        r.description = "seq_count:1 mask_count:{} seq:len:{}".format(mask_count, len(r.seq))
        pass
        log.info("alignment excluded - seq_count:{} after len_variance_filter - {}".format(
                    number_cleaned_seq, basename))
        return
    elif number_cleaned_seq > max_depth_sample:
        selected_records = random.sample(clean_records, max_depth_sample)
    else:
        selected_records = clean_records
    ...
    number_seqs = len(selected_records)
    ...
    if min_SNP_cov:
        if len(selected_records) >= min_SNP_cov:
...
def process_file_list(in_file_list, primer_mismatch, min_base_score, min_seq_score, min_seq_cov,
                      ...
                      min_SNP_cov,indel,max_depth_sample,number_processors=mp.cpu_count()):
    ...
    with mp.Pool(min(len(input_files), number_processors)) as p:
        p.starmap(process_fastq, zip(input_files,primer_F, primer_R,output_dirs,
                                     ...
                                     itertools.repeat(max_depth_sample)
                                     ))
    return output_dirs
    ...
    if _args.mode == "demultiplex":
        ...
        if _args.in_file_list is None:
            if _args.in_file is not None:
                ...
                process_fastq(_args.in_file, _args.primer_forward, _args.primer_reverse, output_dir, _args.primer_mismatch, _args.min_base_score, _args.min_seq_score,
                      ...
                      _args.mask_char, _args.consensus_ambiguous_char,_args.consensus_ignore_mask_char,_args.min_SNP_cov,_args.indel,_args.max_depth_sample)
        ...
        if _args.in_file is not None and _args.oligos is not None:
            ...
            output_dirs = process_file_list(consensus_batch_input, _args.primer_mismatch, _args.min_base_score, _args.min_seq_score,
                          _args.min_seq_cov,_args.max_len,_args.aligner,_args.sequence_max_mask,_args.alignment_max_amb,
                          _args.max_len_delta,_args.expected_length,consensus_threshold,consensus_require_multiple,
                          _args.mask_char,_args.consensus_ambiguous_char,_args.consensus_ignore_mask_char,_args.min_SNP_cov,_args.indel,_args.max_depth_sample,_args.number_processors)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-maxD', '--max_depth_sample', type=int, help='Maximum number of sequences after filtering steps per sample. This filter is applied before haplotype separation. This filter avoids system overloading and excessive runtime. Default: 100000.', default="100000")
